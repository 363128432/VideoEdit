...
- (void) moveSlidersToPosition:(NSNumber *)leftSlider rightSlider: (NSNumber *)rightSlider animated:(BOOL)animated {
    ...
    [UIView transitionWithView:self duration:duration options:UIViewAnimationOptionCurveLinear
                    animations:^(void){
                        self.minHandle.center = CGPointMake(sliderBarWidth * ((float)[leftSlider intValue] / maxValue), sliderBarHeight * 0.5);
                        self.maxHandle.center = CGPointMake(sliderBarWidth * ((float)[rightSlider intValue] / maxValue), sliderBarHeight * 0.5);
                        [self updateValues];
                        //force redraw
